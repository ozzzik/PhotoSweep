//
//  LowQualityListView.swift
//  PhotosCleanup
//
//  Photos older than N years with low quality score (blur, exposure). Review and delete.
//
...
struct LowQualityListView: View {
    ...
    @StateObject private var resultStore = LowQualityResultStore()
    ...
    @State private var minAgeYears: Int = 2
    ...
    @State private var groupPreviewTrigger: ListGroupPreviewTrigger?
    ...
    var body: some View {
        Group {
            if isLoading {
                ProgressView("Loading older photos…")
        ...
        .navigationTitle("Low-quality older photos")
        .toolbar {
        ...
        .confirmationDialog("Delete \(selectedToDelete.count) photos?", isPresented: $showDeleteConfirmation) {
            ...
            Text(DeletionCopy.confirmationMessageModeSpecific(prefix: "These may be accidental or very low quality."))
        }
    ...
    private var emptyDescription: String {
        if effectiveScope != nil {
            return "No older photos below the quality threshold in this range."
        }
        return "Choose scope and minimum age above, then tap Scan."
    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack {
                Text("Minimum age:")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                Picker("Age", selection: $minAgeYears) {
                    Text("2 years").tag(2)
                    Text("3 years").tag(3)
                    Text("5 years").tag(5)
                }
    ...
    private func selectablePhotoRow(index: Int, item: PhotoItem) -> some View {
        ...
        return HStack {
            ...
            Spacer()
            Button {
                groupPreviewTrigger = ListGroupPreviewTrigger(initialIndex: index)
            } label: {
    ...
    private var listContent: some View {
        List {
            Section {
                Text("Review before deleting. These scored low on focus/quality and are older than \(minAgeYears) years. Tap preview to see full size.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Section {
    ...
    private func runScan() async {
        ...
        do {
            let scanScope = scope
            let all = try await library.fetchPhotos(scope: scanScope)
            guard let cutoff = Calendar.current.date(byAdding: .year, value: -minAgeYears, to: Date()) else {
                isLoading = false
                return
            }
            let older = all.filter { ($0.creationDate ?? .distantPast) < cutoff }
            isLoading = false
            if older.isEmpty {
                effectiveScope = scanScope
                if adManager.isPremium {
                    resultStore.save(scope: scanScope, minAgeYears: minAgeYears, assetIds: [])
                }
                return
            }
            isAnalyzing = true
            items = await bestPhotoService.filterLowQuality(from: older, threshold: 0.4) { done, total in
                Task { @MainActor in scanProgress = (done, total) }
            }
            effectiveScope = scanScope
            isAnalyzing = false
            if adManager.isPremium {
                resultStore.save(scope: scanScope, minAgeYears: minAgeYears, assetIds: items.map(\.id))
            }
        } catch {
    ...
    private func loadPersistedIfPremium() async {
        ...
        minAgeYears = persisted.minAgeYears
...
private struct ListGroupPreviewTrigger: Identifiable {
