...
//  PhotosCleanup
//
//  Junk & clutter: receipts, documents, supermarket photos, badly taken. Scope picker + review + delete.
//
...
struct JunkListView: View {
    ...
    @StateObject private var resultStore = JunkResultStore()
    @StateObject private var bestPhotoService: BestPhotoService
    @StateObject private var junkService: JunkDetectionService
    ...
    @State private var groupPreviewTrigger: JunkGroupPreviewTrigger?
    ...
    init(deletionManager: DeletionManager, adManager: AdManager) {
        ...
        _junkService = StateObject(wrappedValue: JunkDetectionService(photoLibrary: lib, bestPhotoService: best))
    }
    ...
    var body: some View {
        Group {
            if isLoading {
                ...
                VStack(spacing: 16) {
                    ...
                    Text("Finding junk \(scanProgress.0) / \(scanProgress.1)")
                        .font(.subheadline)
        ...
        .confirmationDialog("Delete \(selectedToDelete.count) photos?", isPresented: $showDeleteConfirmation) {
            ...
            Text(DeletionCopy.confirmationMessageModeSpecific(prefix: "These look like receipts, documents, or poor-quality shots."))
        }
    ...
    private var emptyDescription: String {
        if let eff = effectiveScope {
            return "No receipts, documents, or badly taken photos found in \(eff.title)."
        }
        return "Choose scope above and tap Scan to find junk photos."
    }
    ...
    private var listContent: some View {
        List {
            Section {
                Text("Receipts, documents, supermarket shots, and badly taken photos. Tap preview to see full size.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Section {
    ...
    private func selectablePhotoRow(index: Int, item: PhotoItem) -> some View {
        ...
        return HStack {
            ...
            Spacer()
            Button {
                groupPreviewTrigger = JunkGroupPreviewTrigger(initialIndex: index)
            } label: {
    ...
    private func runScan() async {
        ...
        do {
            ...
            isAnalyzing = true
            items = await junkService.filterJunk(from: photos) { done, total in
    ...
    private func applyScope(_ newScope: ScanScope) {
        ...
        case .recent(let days):
            scopeOption = .recent
            // This mode uses Recent(30 days) only in the UI; we treat other day values as Recent.
            if days != 30 {
                selectedYear = nil
                customFrom = Calendar.current.date(byAdding: .month, value: -1, to: Date()) ?? Date()
                customTo = Date()
            }
        case .thisMonth:
...
private struct JunkGroupPreviewTrigger: Identifiable {
